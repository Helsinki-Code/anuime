...
```sh
bun --bun ./scripts/new.ts --type registry:ui --name anuime-badge --description "A character-driven status badge."
```
...
```text
registry/items/components/anuime-badge/
  _registry.mdx
  _preview.tsx
  anuime-badge.tsx
```
...
```sh
bun --bun ./scripts/new.ts --type registry:block --name mission-panel --description "A character-driven mission panel."
